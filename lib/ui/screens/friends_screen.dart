...
class _FriendsScreenState extends State<FriendsScreen> {
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: ValueListenableBuilder<bool>(
                  ...
                  builder: (context, loading, _) => loading 
                    ...
                    : IconButton(icon: const Icon(Icons.clear, size: 20), onPressed: () { _searchController.clear(); _controller.searchUsers(""); }),
                ),
                filled: true,
      ...
      builder: (context, results, _) {
        ...
        return Column(
          ...
          children: [
            ...
            ...results.map((user) => _buildUserTile(user, trailing: _buildAddButton(user))),
        ...
        return Column(
          ...
          children: [
            ...
            ...requests.map((request) {
              ...
              final isIncoming = request.initiatorId != _controller.currentUserId;
              return _buildUserTile(
                otherUser,
                subtitle: isIncoming ? 'Sent you a request' : 'Waiting for response',
      ...
      builder: (context, friends, _) {
        return Column(
          ...
          children: [
            ...
            if (friends.isEmpty) 
              const Padding(padding: EdgeInsets.all(32), child: Center(child: Text('No friends yet', style: TextStyle(color: Colors.grey))))
            else
              ...friends.map((friend) => _buildUserTile(
                friend,
  ...
  Widget _buildUserTile(User user, {String? subtitle, Widget? trailing}) {
    return ListTile(
      ...
      subtitle: Text(subtitle ?? user.status.toUpperCase(), style: TextStyle(fontSize: 12, color: Theme.of(context).textTheme.bodySmall?.color?.withValues(alpha: 0.7))),
      trailing: trailing,
    );
  }
  ...
  Widget _buildDefaultAvatar(User user) {
    return CircleAvatar(
      ...
      child: Text(user.nickname[0].toUpperCase(), style: TextStyle(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary)),
    );
  }
  ...
  Widget _buildFriendActions(User friend) {
    return Row(
      ...
      children: [
        IconButton(
          icon: Icon(Icons.chat_bubble_rounded, color: Theme.of(context).colorScheme.primary),
          onPressed: () async {
            final room = await _controller.startDirectChat(friend.id);
            if (room != null && mounted) Navigator.pushReplacementNamed(context, '/home');
          },
        ),
        IconButton(
          icon: Icon(Icons.call_rounded, color: Theme.of(context).colorScheme.primary),
          onPressed: () async {
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              onTap: () { Navigator.pop(context); _controller.removeFriend(friend.id); },
            ),
          ],
        ),
      ),
